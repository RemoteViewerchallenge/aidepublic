...
// src/core/LootboxMcpAdapter.ts
var LootboxMcpAdapter_exports = {};
__export(LootboxMcpAdapter_exports, {
  LootboxMcpAdapter: () => LootboxMcpAdapter
});
module.exports = __toCommonJS(LootboxMcpAdapter_exports);
...
var LootboxRpcClient = class {
  baseUrl;
  constructor(baseUrl) {
    this.baseUrl = baseUrl || process.env.LOOTBOX_URL || "http://host.docker.internal:3005";
  }
  ...
  get fs() {
    return {
      readFile: (args) => this.call("fs", "readFile", args),
      writeFile: (args) => this.call("fs", "writeFile", args),
      listDirectory: (args) => this.call("fs", "listDirectory", args),
      createDirectory: (args) => this.call("fs", "createDirectory", args),
      deleteFile: (args) => this.call("fs", "deleteFile", args),
      moveFile: (args) => this.call("fs", "moveFile", args),
      copyFile: (args) => this.call("fs", "copyFile", args),
      getFileInfo: (args) => this.call("fs", "getFileInfo", args),
      searchReplace: (args) => this.call("fs", "searchReplace", args),
      appendFile: (args) => this.call("fs", "appendFile", args)
    };
  }
  get kv() {
    return {
      get: (args) => this.call("kv", "get", args),
      set: (args) => this.call("kv", "set", args),
      delete: (args) => this.call("kv", "delete", args),
      list: (args) => this.call("kv", "list", args),
      getMany: (args) => this.call("kv", "getMany", args),
      setMany: (args) => this.call("kv", "setMany", args),
      deleteMany: (args) => this.call("kv", "deleteMany", args),
      deletePrefix: (args) => this.call("kv", "deletePrefix", args),
      has: (args) => this.call("kv", "has", args),
      count: (args) => this.call("kv", "count", args),
      clear: (args) => this.call("kv", "clear", args),
      info: (args) => this.call("kv", "info", args),
      reload: (args) => this.call("kv", "reload", args)
    };
  }
  get memory() {
    return {
      createEntities: (args) => this.call("memory", "createEntities", args),
      createRelations: (args) => this.call("memory", "createRelations", args),
      getEntity: (args) => this.call("memory", "getEntity", args),
      search: (args) => this.call("memory", "search", args),
      listTypes: (args) => this.call("memory", "listTypes", args),
      listRelationTypes: (args) => this.call("memory", "listRelationTypes", args),
      deleteEntity: (args) => this.call("memory", "deleteEntity", args),
      deleteRelation: (args) => this.call("memory", "deleteRelation", args),
      getGraph: (args) => this.call("memory", "getGraph", args),
      clear: (args) => this.call("memory", "clear", args),
      stats: (args) => this.call("memory", "stats", args)
    };
  }
  get sqlite() {
    return {
      execute: (args) => this.call("sqlite", "execute", args),
      query: (args) => this.call("sqlite", "query", args),
      transaction: (args) => this.call("sqlite", "transaction", args),
      queryOne: (args) => this.call("sqlite", "queryOne", args),
      listTables: (args) => this.call("sqlite", "listTables", args),
      getSchema: (args) => this.call("sqlite", "getSchema", args),
      vacuum: (args) => this.call("sqlite", "vacuum", args),
      info: (args) => this.call("sqlite", "info", args),
      close: (args) => this.call("sqlite", "close", args)
    };
  }
  get graphql() {
    return {
      query: (args) => this.call("graphql", "query", args),
      mutate: (args) => this.call("graphql", "mutate", args)
    };
  }
};
var LootboxMcpAdapter = class {
  server;
  lootboxClient;
  namespaces = [];
  constructor(lootboxUrl) {
   ...
   */
  async loadManifests() {
    const manifestDir = process.env.MCP_MANIFEST_DIR || "/host/manifests";
    try {
      const entries = await import_promises.default.readdir(manifestDir);
      const namespacesMap = {};
      const prompts = [];
      for (const ent of entries) {
        if (!ent.endsWith(".json")) continue;
        try {
          const raw = await import_promises.default.readFile(import_path.default.join(manifestDir, ent), "utf8");
          ...
          console.warn("Failed to parse manifest", ent, err instanceof Error ? err.message : String(err));
        }
      }
      this.namespaces = Object.keys(namespacesMap).map((ns) => ({ name: ns, functions: namespacesMap[ns] }));
      this._manifestPrompts = prompts;
      if (this.namespaces.length === 0) {
        this.initializeHardcodedNamespaces();
      } else {
        console.error("Loaded manifests, namespaces:", this.namespaces.map((n) => n.name).join(","));
      }
    } catch (err) {
      console.warn("Could not read manifest directory", manifestDir, err instanceof Error ? err.message : String(err));
      this.initializeHardcodedNamespaces();
    }
  }
  initializeHardcodedNamespaces() {
    this.namespaces = [
      {
        name: "code",
        functions: [
  ...
  async loadNamespaceFunctions(namespace) {
    try {
      ...
      while ((match = functionRegex.exec(typesContent)) !== null) {
        ...
        if (ns.toLowerCase() === namespace.name) {
          ...
          const resultDescription = resultMatch ? resultMatch[1].trim() : "No description available";
          functions.push({
  ...
  setupHandlers() {
      ...
      async (request) => {
        ...
        try {
          ...
          if (namespace === "code" && method === "generate") {
            const codeText = result?.code ?? result?.text ?? JSON.stringify(result, null, 2);
            return {
              content: [
                {
                  type: "text",
          ...
          throw new McpError(
            ...
            `Lootbox tool execution failed: ${error instanceof Error ? error.message : String(error)}`
          );
        }
      }
    );
    this.server.setRequestHandler(
      ListPromptsRequestSchema,
      async (request) => {
        const prompts = this._manifestPrompts || [];
        return {
          prompts,
          page: 1,
    ...
    this.server.setRequestHandler(
      ...
      async (request) => {
        ...
        const found = prompts.find((p) => p.id === name || p.title === name);
        if (!found) {
          throw new McpError(
...
var adapter = new LootboxMcpAdapter();
