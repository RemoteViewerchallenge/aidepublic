...
// LLM exports removed - using custom adapters instead
// Telemetry exports removed - not needed for orchestration
// export { createVolcanoTelemetry, noopTelemetry } from './telemetry';
// export type { VolcanoTelemetry, VolcanoTelemetryConfig } from './telemetry';

/* ---------- LLM ---------- */
export type { LLMHandle, LLMToolResult, ToolDefinition };
// NOTE: llmOpenAI exports removed since we use custom adapters (GeminiAdapter, OpenRouterAdapter)
...
function normalizeError(
  ...
  if (kind === 'retry')
    return new RetryExhaustedError(
      e?.message || 'Retry attempts exhausted',
      { ...meta },
      { cause: e }
    );
  if (kind === 'llm') {
  ...
  if (kind === 'mcp-conn') {
    ...
    return new MCPConnectionError(
      e?.message || 'MCP connection error',
      { ...meta, retryable },
      { cause: e }
    );
  }
...
export function __internal_validateToolArgs(schema: any, args: any) {
  validateWithSchema(schema, args, 'test');
}
...
async function getOAuthToken(
  ...
  if (!auth.tokenEndpoint || !auth.clientId || !auth.clientSecret) {
    throw new Error(
      `OAuth auth requires tokenEndpoint, clientId, and clientSecret`
    );
  }
...
async function getPooledClient(
  ...
  if (!entry) {
    ...
    if (MCP_POOL.size >= MCP_POOL_MAX) {
      const idleEntries = Array.from(MCP_POOL.entries()).filter(
        ([, e]) => e.busyCount === 0
      );
      idleEntries.sort((a, b) => a[1].lastUsed - b[1].lastUsed);
      const toEvict = idleEntries.slice(
        0,
        Math.max(0, MCP_POOL.size - MCP_POOL_MAX + 1)
      );
...
export function __internal_getMcpPoolStats() {
  return {
    size: MCP_POOL.size,
    entries: Array.from(MCP_POOL.entries()).map(([url, e]) => ({
      url,
...
export async function __internal_forcePoolCleanup() {
  await cleanupIdlePool();
}
export function __internal_setPoolConfig(max: number, idleMs: number) {
  MCP_POOL_MAX = max;
  MCP_POOL_IDLE_MS = idleMs;
}
export function __internal_clearOAuthTokenCache() {
...
async function withMCP<T>(
  ...
  const mcpSpan =
    telemetry && operation ? telemetry.startMCPSpan(null, h, operation) : null;
...
// Normalize different MCP client.listTools() shapes. Some MCP clients return
// an array directly while others return an object with a `tools` property.
function extractToolsFromClientResult(result: any) {
  if (!result) return [];
  if (Array.isArray(result)) return result;
  if (Array.isArray(result.tools)) return result.tools;
  // fallback: try common alternative property names
  if (Array.isArray(result.toolDefinitions)) return result.toolDefinitions;
  return [];
}
  ...
  for (const handle of handles) {
    try {
      ...
      const tools = await withMCP(handle, async client => {
        ...
        const rawTools = extractToolsFromClientResult(result);
        const mapped = rawTools.map((tool: any) => ({
...
export function __internal_clearDiscoveryCache() {
  TOOL_CACHE.clear();
}
export function __internal_setDiscoveryTtl(ms: number) {
  TOOL_CACHE_TTL_MS = ms;
}
export function __internal_primeDiscoveryCache(
  handle: MCPHandle,
...
async function getToolSchema(
  ...
  const cached = TOOL_CACHE.get(handle.url);
  if (cached) {
    const found = cached.tools.find(
      (t: any) => t.name === `${handle.id}.${toolName}`
    );
  ...
  try {
    ...
    const found = tools.find((t: any) => t.name === `${handle.id}.${toolName}`);
    return found?.parameters as any;
  } catch {
    return undefined;
  }
...
export interface AgentBuilder {
  ...
  parallel(
    stepsOrDict: Step[] | Record<string, Step>,
    hooks?: { pre?: () => void; post?: () => void }
  ): AgentBuilder;
  branch(
  ...
  switch<T = string>(
    ...
    cases: Record<string, (agent: AgentBuilder) => AgentBuilder> & {
  ...
  stream(
    optionsOrLog?: StreamOptions | ((s: StepResult, stepIndex: number) => void)
  ): AsyncGenerator<StepResult, void, unknown>;
}

function buildHistoryContextChunked(
  history: StepResult[],
...
const createProgressDisplay = (workflowStart: number, isTTY: boolean) => ({
  ...
  showTokens: (count: number, provider?: string) => {
    ...
    if (count % 10 === 0 || count === 1) {
      process.stdout.write(
        `\r   💭 ${count} tokens | ${throughput} tok/s${providerInfo}`
      );
    }
  },
  ...
  showComplete: (durationMs: number, tokens: number, provider?: string) => {
    ...
    console.log(
      `   ✅ Complete | ${tokens.toLocaleString()} token${
        tokens > 1 ? 's' : ''
      } | ${(durationMs / 1000).toFixed(1)}s${providerInfo}\n`
    );
  },
});
...
function createProgressHandler(
  ...
  return {
    stepStart: (stepIndex: number, prompt?: string) => {
      ...
      const actualStepNum =
        isExplicitSubAgent && parentStepIndex !== undefined
          ? parentStepIndex + stepIndex + 1
          : stepIndex + 1;
      const actualTotalSteps =
        isExplicitSubAgent && parentTotalSteps !== undefined
          ? parentTotalSteps
          : totalSteps;
      console.log(
        `🤖 Step ${actualStepNum}/${actualTotalSteps}: ${display}${
    ...
    llmToken: (count: number, provider?: string) => {
      ...
      if (count % 10 === 0 || count === 1) {
        const providerInfo = provider ? ` | ${provider}` : '';
        process.stdout.write(
          `\r   💭 ${count} tokens | ${throughput} tok/s | ${elapsed.toFixed(
            1
          )}s${providerInfo}`
        );
      }
    ...
    workflowEnd: (
      ...
      if (totalTokens && models && models.length > 0) {
        ...
        console.log(
          `🎉 Agent complete | ${totalTokens.toLocaleString()} tokens | ${(
            (totalDuration || 0) / 1000
          ).toFixed(1)}s | ${modelsList}`
        );
      } else {
        const total = (Date.now() - workflowStart) / 1000;
        console.log(
          `🎉 Workflow complete! ${stepCount} step${
            stepCount > 1 ? 's' : ''
...
export function agent(opts?: AgentOptions): AgentBuilder {
  ...
  const defaultTimeoutMs =
    (typeof opts?.timeout === 'number' ? opts!.timeout! : 60) * 1000; // seconds -> ms
  const defaultRetry: RetryConfig = opts?.retry ?? { delay: 0, retries: 3 };
  const contextMaxChars =
    typeof opts?.contextMaxChars === 'number' ? opts!.contextMaxChars! : 20480;
  const contextMaxToolResults =
    typeof opts?.contextMaxToolResults === 'number'
      ? opts!.contextMaxToolResults!
      : 8;
  const agentMcpAuth = opts?.mcpAuth || {};
  const telemetry = opts?.telemetry;
  const defaultMaxToolIterations =
    typeof opts?.maxToolIterations === 'number' ? opts!.maxToolIterations! : 4;
  let isRunning = false;
  ...
  const builder: AgentBuilder = {
    ...
    resetHistory() {
      steps.push({ __reset: true });
      return builder;
    },
    then(s: Step | StepFactory) {
      steps.push(s);
      return builder;
    },
    ...
    async run(
      log?: (s: StepResult, stepIndex: number) => void
    ): Promise<StepResult[]> {
      if (isRunning) {
        throw new AgentConcurrencyError(
          'This agent is already running. Create a new agent() instance for concurrent runs.'
        );
      }
      isRunning = true;
      ...
      // Start agent span
      const agentSpan =
        telemetry?.startAgentSpan(steps.length, agentName) || null;
      ...
      try {
        ...
        for (const raw of planned) {
          ...
          if ((raw as any).__branch) {
            ...
            const branchResults = await executeBranch(
              condition,
              branches,
              out,
              () => agent(opts)
            ...
            branchResults.forEach((r, i) =>
              log?.(r, out.length - branchResults.length + i)
            );
          ...
          if ((raw as any).__switch) {
            ...
            const switchResults = await executeSwitch(
              selector,
              cases,
              out,
              () => agent(opts)
            ...
            switchResults.forEach((r, i) =>
              log?.(r, out.length - switchResults.length + i)
            );
          ...
          if ((raw as any).__while) {
            ...
            const whileResults = await executeWhile(
              condition,
              body,
              out,
              () => agent(opts),
            ...
            whileResults.forEach((r, i) =>
              log?.(r, out.length - whileResults.length + i)
            );
          ...
          if ((raw as any).__forEach) {
            ...
            const forEachResults = await executeForEach(items, body, () =>
              agent(opts)
            );
            out.push(...forEachResults);
            contextHistory.push(...forEachResults);
            forEachResults.forEach((r, i) =>
              log?.(r, out.length - forEachResults.length + i)
            );
          ...
          if ((raw as any).__retryUntil) {
            ...
            const retryResults = await executeRetryUntil(
              body,
              successCondition,
              () => agent(opts),
              retryOpts
            ...
            retryResults.forEach((r, i) =>
              log?.(r, out.length - retryResults.length + i)
            );
          ...
          if ((raw as any).__runAgent) {
            ...
            const subResults = await executeRunAgent(
              subAgent,
              out.length,
              planned.length,
              contextHistory
            ...
            subResults.forEach((r, i) =>
              log?.(r, out.length - subResults.length + i)
            );
          ...
          const s =
            typeof raw === 'function'
              ? (raw as StepFactory)(out)
              : (raw as Step);
          const stepTimeoutMs =
            (s as any).timeout != null
              ? (s as any).timeout * 1000
              : defaultTimeoutMs; // seconds -> ms
          const retryCfg: RetryConfig = (s as any).retry ?? defaultRetry;
          const attemptsTotal =
            typeof retryCfg.retries === 'number' && retryCfg.retries! > 0
              ? retryCfg.retries!
              : defaultRetry.retries ?? 3;
          const useDelay =
            typeof retryCfg.delay === 'number'
              ? retryCfg.delay!
              : defaultRetry.delay ?? 0;
          ...
          const doStep = async (): Promise<StepResult> => {
            ...
            if ('mcps' in s && 'prompt' in s) {
              ...
              if (!usedLlm)
                throw new Error(
                  'No LLM provided. Pass { llm } to agent(...) or specify per-step.'
                );
              const stepInstructions =
                (s as any).instructions ?? globalInstructions;
              const cmr =
                (s as any).contextMaxToolResults ?? contextMaxToolResults;
              const cmc = (s as any).contextMaxChars ?? contextMaxChars;
              const promptWithHistory =
                (s as any).prompt +
                buildHistoryContextChunked(contextHistory, cmr, cmc);
              r.prompt = (s as any).prompt;
              // Apply agent-level auth to all MCP handles
              const mcpsWithAuth = ((s as any).mcps as MCPHandle[]).map(
                applyAgentAuth
              );
              ...
              if (availableTools.length === 0) {
                ...
                const maxIterations =
                  (s as any).maxToolIterations ?? defaultMaxToolIterations;
                let workingPrompt =
                ...
                for (let i = 0; i < maxIterations; i++) {
                  ...
                  try {
                    toolPlan = await usedLlm.genWithTools(
                      workingPrompt,
                      availableTools
                    );
                  } catch (e) {
                    const provider = classifyProviderFromLlm(usedLlm);
                    throw normalizeError(e, 'llm', {
                      stepId: out.length,
                      provider,
                    });
                  }
                  ...
                  telemetry?.recordMetric('llm.call', 1, {
                    provider: (usedLlm as any).id || usedLlm.model,
                    error: false,
                  });
                  telemetry?.recordMetric('llm.duration', llmCallDuration, {
                    provider: (usedLlm as any).id || usedLlm.model,
                    model: usedLlm.model,
                  });
                  ...
                  if (
                    !toolPlan ||
                    !Array.isArray(toolPlan.toolCalls) ||
                  ...
                  for (const call of toolPlan.toolCalls) {
                    ...
                    const actualToolName =
                      idx >= 0 ? mapped.name.slice(idx + 1) : mapped.name;
                    ...
                    try {
                      ...
                      throw normalizeError(e, 'mcp-tool', {
                        stepId: out.length,
                        provider,
                      });
                    }
            ...
            // Automatic agent selection with iterative delegation
              ...
              if (
                ...
                !availableAgents.some(a => a.name && a.description)
              ) {
                r.llmOutput =
                  'No agents available or agents missing name/description.';
              } else {
                const agentContext = buildAgentContext(availableAgents);
                const maxIterations =
                  (s as any).maxAgentIterations ?? defaultMaxToolIterations;
                let workingPrompt =
                  (stepInstructions ? stepInstructions + '\n\n' : '') +
                  promptWithHistory +
                  agentContext;
                ...
                for (let i = 0; i < maxIterations; i++) {
                  ...
                  try {
                    ...
                    if (progress && typeof usedLlm.genStream === 'function') {
                      const tokens: string[] = [];
                      for await (const token of usedLlm.genStream(
                        workingPrompt
                      )) {
                        tokens.push(token);
                        coordTokenCount++;
                        progress.llmToken(
                          coordTokenCount,
                          (usedLlm as any).id || usedLlm.model
                        );
                      }
                  ...
                  if (decision.type === 'done') {
                    ...
                    if (progress) {
                      ...
                      if (i === 0) {
                        process.stdout.write(
                          '🧠 Coordinator: Final answer ready\n'
                        );
                      } else {
                        process.stdout.write(
                          '🧠 Coordinator: Final answer ready\n'
                      ...
                      process.stdout.write(
                        `   ✅ Complete | ${coordTokenCount} tokens | ${coordTime.toFixed(
                          1
                        )}s | ${(usedLlm as any).id || usedLlm.model}\n`
                      );
                    }
                    ...
                    if (progress) {
                      ...
                      if (i === 0) {
                        process.stdout.write(
                          `🧠 Coordinator decision: USE ${decision.agentName}\n`
                        );
                      } else {
                        process.stdout.write(
                          `🧠 Coordinator decision: USE ${decision.agentName}\n`
                    ...
                    if (!selectedAgent) {
                      workingPrompt += `\n\nError: Agent '${
                        decision.agentName
                      }' not found. Available: ${availableAgents
                        .map(a => a.name)
                        .join(', ')}`;
                      continue;
                    }
                    ...
                    try {
                      ...
                      (delegatedAgent as any).__parentAgentName =
                    ...
                    const agentOutput =
                      agentResult[agentResult.length - 1]?.llmOutput ||
                      '[no output]';
                    agentCalls.push({
                ...
                if (agentCalls.length > 0) {
                  ...
                  telemetry?.recordMetric(
                    'agent.delegation',
                    agentCalls.length,
                    { agents: agentCalls.map(c => c.name).join(',') }
                  );
                  for (const agentCall of agentCalls) {
                    telemetry?.recordMetric('agent.call', 1, {
            ...
            // LLM-only steps
              ...
              const promptWithHistory =
                (s as any).prompt +
                buildHistoryContextChunked(contextHistory, cmr2, cmc2);
              const finalPrompt =
                (stepInstructions ? stepInstructions + '\n\n' : '') +
                promptWithHistory;
              r.prompt = (s as any).prompt;
              const llmSpan =
                telemetry?.startLLMSpan(stepSpan, usedLlm, finalPrompt) || null;
              if (progress) progress.startLlmOperation();
              const llmStart = Date.now();
              try {
                let tokenCount = 0;
                const customOnToken = (s as any).onToken;
                const progressOnToken =
                  !customOnToken && progress
                    ? () => {
                        tokenCount++;
                        progress.llmToken(
                          tokenCount,
                ...
                telemetry?.recordMetric('error', 1, {
                  type: 'llm',
                  provider: (usedLlm as any).id || usedLlm.model,
                });
                const provider = classifyProviderFromLlm(usedLlm);
            ...
            // Explicit MCP tool calls (existing behavior)
              ...
              if ('prompt' in s) {
                ...
                const promptWithHistory =
                  (s as any).prompt +
                  buildHistoryContextChunked(contextHistory, cmr3, cmc3);
                const finalPrompt =
                  (stepInstructions ? stepInstructions + '\n\n' : '') +
                  promptWithHistory;
                r.prompt = (s as any).prompt;
              ...
              validateWithSchema(
                schema,
                (s as any).args ?? {},
                `Tool ${mcpHandle.id}.${(s as any).tool}`
              );
              const mcpStart = Date.now();
              let res: any;
              try {
            ...
            telemetry?.recordMetric('step.duration', r.durationMs, {
              type: stepType,
            });
          ...
          for (let attempt = 1; attempt <= attemptsTotal; attempt++) {
            try {
              ...
              if (e instanceof Error && /timed out/i.test(e.message)) {
                ...
                /failed schema validation/i.test(
              ...
              lastError = vErr || e;
              if (
                lastError instanceof VolcanoError &&
                lastError.meta?.retryable === false
              ) {
          ...
          if (!result)
            throw lastError instanceof VolcanoError
              ? lastError
        ...
        if (out.length > 0) {
          const totalDuration = out.reduce(
          ...
          telemetry?.recordMetric('agent.duration', totalDuration, {
            steps: out.length,
          });
          telemetry?.recordMetric('workflow.steps', out.length, {
            agent_name: agentName || 'anonymous',
          });
        }
        return out;
        ...
        if (progress) {
          ...
          const totalTokens = out.reduce((acc, s) => {
            const stepTokens =
              (s as any).__tokenCount || (s as any).__crewTotalTokens || 0;
          ...
          out.forEach(s => {
            ...
            if (crewModels)
              crewModels.forEach((m: string) => modelsUsed.add(m));
          });
          const totalDuration = out.reduce(
            (acc, s) => acc + (s.durationMs || 0),
            0
          );
          progress.workflowEnd(
            steps.length,
            totalTokens,
            totalDuration,
    ...
    async *stream(
      optionsOrLog?:
        | StreamOptions
        | ((s: StepResult, stepIndex: number) => void)
    ): AsyncGenerator<StepResult, void, unknown> {
      if (isRunning) {
        throw new AgentConcurrencyError(
          'This agent is already running. Create a new agent() instance for concurrent runs.'
      ...
      let streamOnToken:
        | ((token: string, meta: TokenMetadata) => void)
        | undefined;
        ...
        for (const raw of planned) {
          ...
          if ((raw as any).__branch) {
            ...
            for (const r of branchResults) {
              log?.(
                r,
                out.length - branchResults.length + branchResults.indexOf(r)
              );
              yield r;
            }
          ...
          if ((raw as any).__switch) {
            ...
            for (const r of switchResults) {
              log?.(
                r,
                out.length - switchResults.length + switchResults.indexOf(r)
              );
              yield r;
            }
          ...
          if ((raw as any).__while) {
            ...
            for (const r of whileResults) {
              log?.(
                r,
                out.length - whileResults.length + whileResults.indexOf(r)
              );
              yield r;
            }
          ...
          if ((raw as any).__forEach) {
            ...
            for (const r of forEachResults) {
              log?.(
                r,
                out.length - forEachResults.length + forEachResults.indexOf(r)
              );
              yield r;
            }
          ...
          if ((raw as any).__retryUntil) {
            ...
            for (const r of retryResults) {
              log?.(
                r,
                out.length - retryResults.length + retryResults.indexOf(r)
              );
              yield r;
            }
