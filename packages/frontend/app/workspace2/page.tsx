...
const testAPI = async (): Promise<any> => {
  try {
    ...
    console.error('API Test Failed:', _error);
    return null;
  }
};
...
export default function Workspace2() {
  ...
  const startMcp = async () => {
    ...
    try {
      const res = await fetch('/api/start-mcp', { method: 'POST' });
      const data = await res.json();
      if (!res.ok || !data.ok) {
        setMcpStatus(
          `Failed to start: ${data?.error || data?.status || res.status}`
        );
      ...
      const poll = async () => {
        try {
          const s = await fetch('/api/start-mcp');
          const sj = await s.json();
          if (sj.ok) {
            setMcpStatus('MCP is reachable');
            setMcpStarting(false);
            return;
          }
        } catch (_e) {
        ...
        if (Date.now() - start > timeout) {
          setMcpStatus('Timed out waiting for MCP to become ready');
          setMcpStarting(false);
          return;
        }
      ...
      setMcpStatus('Start request failed');
      setMcpStarting(false);
    }
  };
  ...
  useEffect(() => {
    const getStatus = async () => {
      try {
        ...
        console.error('Failed to get provider status:', _error);
      }
    };
    getStatus();
  }, []);
  ...
  useEffect(() => {
    // Load persisted roles from roles service
    const loadRoles = async () => {
      try {
        const res = await fetch('http://localhost:4600/roles');
        if (res.ok) {
          const roles = await res.json();
          setSavedRoles(roles || []);
          const allRoles = [currentRole, ...(roles || [])];
          const uniqueRoles = Array.from(
            new Map(allRoles.map(role => [role.id, role])).values()
          );
          setAvailableRoles(uniqueRoles);
        } else {
          console.warn('Roles service returned', res.status);
        }
      } catch (_error) {
        console.error('Failed to load roles from roles service:', _error);
      }
    };
    loadRoles();
  }, [currentRole]);
    ...
    (async () => {
      try {
        ...
        if (res.ok) {
          ...
          setSavedRoles(prev => [...prev, saved]);
          setAvailableRoles(prev => {
            const map = new Map(prev.map(r => [r.id, r]));
            map.set(saved.id, saved);
            return Array.from(map.values());
          });
          alert('Role persisted to roles service');
        } else {
          const text = await res.text();
          console.error('Roles service error:', res.status, text);
          alert('Failed to persist role');
        }
      } catch (_error) {
  ...
  const generateWithLLM = async (
    ...
    try {
      ...
      // Prepare model hints: if a model was pinned for this role in this session, send it as preferredModelId
      const sessionKey = `roleModel:${role.id}`;
      const preferred = sessionStorage.getItem(sessionKey);
      ...
      if (preferredModelId) requestBody.preferredModelId = preferredModelId;
      const blacklistKey = `roleBlacklist:${role.id}`;
      const blacklistedJson = sessionStorage.getItem(blacklistKey);
      if (blacklistedJson) {
        try {
          const arr = JSON.parse(blacklistedJson);
          if (Array.isArray(arr) && arr.length > 0)
            requestBody.blacklistedModelIds = arr;
        } catch (_e) {
          // ignore
        }
      }
      ...
      if (data.error) {
        ...
        // If an error references a model id, add it to session blacklist so next retry avoids it
        try {
          const maybeModel = data?.modelId || data?.providerId || null;
          if (maybeModel) {
            const blacklistKey = `roleBlacklist:${role.id}`;
            const prev = JSON.parse(
              sessionStorage.getItem(blacklistKey) || '[]'
            );
            if (!prev.includes(maybeModel)) {
              prev.push(maybeModel);
              sessionStorage.setItem(blacklistKey, JSON.stringify(prev));
            }
          }
        } catch (_e) {
          // ignore
      ...
      // Persist the successful model for this role in this session only
      try {
        if (actualResult.modelId) {
          const sessionKey = `roleModel:${role.id}`;
          sessionStorage.setItem(
            sessionKey,
            JSON.stringify({
              modelId: actualResult.modelId,
  ...
  return (
    ...
    >
      {/* Title removed to maximize workspace (requested) */}
      ...
      <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
          ...
          >
            ...
            >
              ...
              <div>
                ...
                {/* Start MCPJungle button + status */}
                <div style={{ marginTop: '10px', marginBottom: '8px' }}>
                  <button
                    onClick={startMcp}
                    disabled={mcpStarting}
                    style={{
                      background: mcpStarting ? '#666' : '#28a745',
                      color: 'white',
                      border: 'none',
                      padding: '6px 12px',
                      borderRadius: '4px',
                  ...
                  >
                    {mcpStarting ? 'Starting MCP...' : 'Start MCPJungle'}
                  </button>
                  {mcpStatus && (
                ...
                {/* RoleMcpSelector moved to the right-hand compact sidebar to save space */}
              </div>
              ...
              <div>
                ...
                <div style={{ marginBottom: '15px' }}>
                  ...
                  <div style={{ fontSize: '12px' }}>
                    <div>
                      <strong>{modelStats.matching}</strong> /{' '}
                      {modelStats.total} models match
                    </div>
        ...
        {/* Right sidebar: compact Role->Tool selector (~1/4 width) */}
        <div style={{ width: 300, flexShrink: 0 }}>
          <RoleMcpSelector roleId={currentRole.id} />
        </div>
      </div>
