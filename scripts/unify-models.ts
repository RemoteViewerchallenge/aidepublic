...
async function unifyModels() {
  ...
  try {
    ...
    for (const model of aiStudioResult.rows) {
      ...
      const hasVision = modelName.includes('vision') || modelName.includes('image') || modelId.includes('vision') || modelId.includes('gemini');
      const hasThinking = model.thinking || modelName.includes('think') || modelName.includes('reason') || modelName.includes('experiment');
      const hasEmbedding = model.supportedGenerationMethods?.includes('embedContent') || modelName.includes('embed');
      const hasTools = model.supportedGenerationMethods?.includes('generateContent') || model.supportedGenerationMethods?.includes('toolUse');
    ...
    const openRouterResult = await client.query('SELECT * FROM openrouter_models');
    for (const model of openRouterResult.rows) {
        const modelName = (model.name || '').toLowerCase();
        const modelId = (model.id || '').toLowerCase();
        const hasVision = modelName.includes('vision') || modelName.includes('image') || modelId.includes('vision') || model.architecture?.input_modalities?.includes('image');
        const hasThinking = modelName.includes('think') || modelName.includes('reason') || modelName.includes('o1') || modelName.includes('experiment');
        const hasEmbedding = modelName.includes('embed') || modelId.includes('embed');
        const hasTools = model.supported_parameters?.includes('tools') || model.supported_parameters?.includes('functions');
